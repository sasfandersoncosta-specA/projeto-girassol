    ...
    function createSlideHTML(questionData, index) { 
        ...
        switch (questionData.type) { 
            ...
            case 'choice': case 'multiple-choice': 
                ...
                break;
            case 'rating':
                contentHTML = `<div class="rating-stars">
   ...
   if (currentQuestion.id === 'idade' && target.dataset.value === 'Menor de 18 anos') {
            ...
            if (currentQuestion.id === 'modalidade_atendimento') {
                ...
                if (modalidade === 'Online') {
                    ...
                    userAnswers['cep'] = null; // Limpa o CEP se o usuário voltar e mudar
                }
            }
            setTimeout(() => goToSlide(proximoPasso), 250);
        } } } if (target.matches('.rating-stars label')) { setTimeout(() => { updateNamePlaceholders(userAnswers.nome); goToSlide(questions.findIndex(q => q.id === 'agradecimento')); setTimeout(finalize, 1500); }, 250); } }); slidesContainer.addEventListener('keydown', (e) => { if (e.key === 'Enter' && e.target.matches('.text-input')) { e.preventDefault(); validateAndAdvance(); } }); if (document.querySelector(`[data-index="0"]`)) { goToSlide(0); } else { console.error("Erro: Slide inicial não encontrado."); } }
    initializeQuiz();
