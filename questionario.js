    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (target.matches('.back-button')) {
            ...
            goToSlide(passoAnterior); // <-- CORRIGIDO: Agora está DENTRO do 'if'
        } 
        ...
        if (target.matches('.choice-button')) { 
            ...
            if (isMulti) { 
                ...
                if (currentQuestion.id === 'idade' && target.dataset.value === 'Menor de 18 anos') {
                    ...
                    // Se não for menor de 18, continua o fluxo normal
        ...
        // --- 4. Ação: Clicar no Rating de Estrelas ---
        if (target.matches('.rating-stars label')) { 
            setTimeout(() => { 
                updateNamePlaceholders(userAnswers.nome); 
                goToSlide(questions.findIndex(q => q.id === 'agradecimento')); 
                setTimeout(finalize, 1500); 
            }, 250); 
        } 
    }); slidesContainer.addEventListener('keydown', (e) => { if (e.key === 'Enter' && e.target.matches('.text-input')) { e.preventDefault(); validateAndAdvance(); } }); if (document.querySelector(`[data-index="0"]`)) { goToSlide(0); } else { console.error("Erro: Slide inicial não encontrado."); } }
        initializeQuiz();
